...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Si hay búsqueda, no filtrar por activos (permite ver inactivos en resultados)
        // Si no hay búsqueda, solo mostrar activos
        if ($request->filled("se")) {
            $filters['search'] = $request->query("se");
            // No aplicar filtro de active cuando hay búsqueda
        } else {
            // Solo mostrar activos cuando no hay búsqueda
            $filters['active'] = 1;
        }
